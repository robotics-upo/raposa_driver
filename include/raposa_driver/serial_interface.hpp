...
#ifndef __SERIAL_IFACE_HPP__
#define __SERIAL_IFACE_HPP__

#ifndef _RAPOSA_SERIAL_DEBUG_
...
//! TODO: Check and test all. See necessities of raposa comms
...
class SerialInterface
{
  ...
  virtual bool open(uint32_t baudrate = 115200, uint8_t mode = O_RDWR | O_NOCTTY | O_NDELAY);
...
inline int SerialInterface::read_stream(unsigned char* buffer, int buffer_size)
{
  ...
  while (bytes < buffer_size && bytes >= 0) {
