...
{
  // ------------- Init ROS stuff -----------------------
  ros::init(argc, argv, "RaposaCalibration");
  ros::NodeHandle n;
  ros::NodeHandle pn("~");
  
  ROS_INFO("Starting Raposa calibration node. ");
  ...
  std::string raposa_port("/dev/serial/by-id/usb-FTDI_MM232R_USB_MODULE_FTB3LEN7-if00-port0");
  ...
  try {
    ...
    ROS_ERROR("Could not connect to Raposa. Exiting.\n");
    return -1;
  }
  
  // Before starting --> stop Raposa
  raposa->setRawVelocity(0, 0);
  ...
  if (!twist) {
	...
	command_right *= -1;
      ...
      ROS_INFO("Raposa calibration. Sending L = %d and R = %d during %f seconds. Press RED (Panic) button to to cancel or START to proceed in 2 secs.", command_left, command_right, total_time);
	...
	return -2;
      ...
      t = 0.0;
      raposa->resetOdometry();
      
      while (n.ok() && t < total_time) {
	raposa->setRawVelocity(command_left, command_right);
	r.sleep();
	t += delta_t;
	raposa->update();
	ros::spinOnce();
      ...
      cout << "Left ticks: " << st.total_left_ticks << endl;
      cout << "Right ticks: " << st.total_right_ticks << endl;
	...
	command_right *= -1;
      ...
      ROS_INFO("Raposa calibration. Twist test. Sending L = %d and R = %d during %f seconds. Press RED (Panic) button to to cancel or START to proceed in 2 secs.", command_left, command_right, total_time);
